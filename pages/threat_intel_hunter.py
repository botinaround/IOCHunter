"""
IOC Hunter — Main threat intelligence analysis page.
"""
...
def render_results(data: dict, source_url: str):
            ...
            with st.expander(f"{label} ({len(entries)})", expanded=len(entries) <= 10):
                ...
                for e in entries:
                    if isinstance(e, dict):
                        ...
                        rows.append({"Confidence": f"{icon} {conf}", "Value": e.get("value", ""), "Context": e.get("context", "")})
                    else:
    ...
    with tab_mitre:
        ...
        if techniques:
            st.dataframe(
                [{"ID": t.get("id",""), "Name": t.get("name",""), "Context": t.get("context","")} for t in techniques],
...
def render_threat_hunt(data: dict):
    ...
    with tab_detections:
        ...
        if detections:
            st.dataframe(
                [{"Technique": d.get("technique",""), "Behaviour": d.get("description",""), "Log Source": d.get("log_source","")} for d in detections],
                use_container_width=True, hide_index=True,
            )
        else:
    ...
    if from_cache:
        ...
        with st.status("Running IOC Hunter...", expanded=True) as status:
            if mode == "Paste Text":
                ...
                st.write(f"Scraped **{scraped['title']}** — {len(scraped['text']):,} characters")
            ...
            if report_type in ("Technical IOC Report", "Threat Hunt Report"):
                ...
                st.write(f"Found **{total_candidates}** IOC candidates across **{len(pre)}** types")
